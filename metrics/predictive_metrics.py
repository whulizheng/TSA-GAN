...
def predictive_score_metrics(ori_data, generated_data):
    ...
    def predictor(x, t):
        ...
        with tf.variable_scope("predictor", reuse=tf.AUTO_REUSE) as vs:
            ...
            p_vars = [v for v in tf.all_variables(
    ...
    for itt in range(iterations):
        ...
        Y_mb = list(np.reshape(
            generated_data[i][1:, (dim-1)], [len(generated_data[i][1:, (dim-1)]), 1]) for i in train_idx)

        # Train predictor
        _, step_p_loss = sess.run([p_solver, p_loss], feed_dict={
                                  X: X_mb, T: T_mb, Y: Y_mb})
    ...
    Y_mb = list(np.reshape(
        ori_data[i][1:, (dim-1)], [len(ori_data[i][1:, (dim-1)]), 1]) for i in train_idx)
    ...
    for i in range(no):
        MAE_temp = MAE_temp + \
            mean_absolute_error(Y_mb[i], pred_Y_curr[i, :, :])
